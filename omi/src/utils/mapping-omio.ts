/**
 * mappingjs v2.0.0 by dntzhang (Omio-compatible version)
 * Objects mapping for javascript. Omi MVVM's best partner.
 * Includes obaa (observable array) compatibility.
 */
...
function arrayMapping(from: unknown[], to: ObservableArray<unknown>, rule: MappingRule, path: string): void {
  ...
  from.forEach((item, index) => {
    if (index > to.length - 1) {
      // Use push to trigger obaa callback
      if (isArray(item)) {
        to.push(arrayMapping(item as any, (to[index] || []) as any, rule, `${path}[${index}]`));
      } else if (isObject(item)) {
        to.push(objMapping(item, to[index] as AnyObject, rule, `${path}[${index}]`));
      } else {
        to.push(item);
      }
    } else {
 ...
 * Omio-compatible version with obaa support.
 */
